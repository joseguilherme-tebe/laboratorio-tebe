...
# Layout Padrão para Gráficos (Mantendo coerência visual)
layout_default = dict(margin=dict(l=20, r=20, t=40, b=20))

# ==============================================================================
# MÓDULO 1: FUNDAMENTOS (Aceleração, Velocidade, Deslocamento)
# ==============================================================================
with tab1:
    ...
    with col2:
        ...
        if viz_mode == "Formas de Onda (Tempo)":
            ...
            # Cálculo RMS Real para Linha Tracejada
...
with tab2:
    ...
    with col_cfg:
        ...
        num_linhas = st.selectbox("Número de Linhas (Resolução)", [400, 800, 1600, 3200, 6400, 12800], index=0) # Index 0 para facilitar teste de baixa resolução
    ...
    with col_vis:
        ...
        # Simulação Digital: O PULO DO GATO PARA VISUALIZAR RESOLUÇÃO
        # Precisamos simular que coletamos APENAS o número de pontos correspondente ao tempo de coleta
        n_samples_coleta = int(tempo_coleta * amostragem)
        
        # Sinal Analógico (Alta resolução para fundo)
        t_analog = np.linspace(0, tempo_coleta, 10000) 
        y_analog = np.sin(2 * np.pi * sig_freq * t_analog)
        
        # Sinal Digital (Exatamente os pontos coletados)
        t_digital = np.arange(n_samples_coleta) / amostragem
        y_digital = np.sin(2 * np.pi * sig_freq * t_digital)
        
        # Gráfico 2: Forma de Onda
        fig_dig = go.Figure()
        # Mostra apenas um trecho para não poluir
        ...
        # Gráfico 3: Espectro (CORRIGIDO PARA RESOLUÇÃO REAL)
        # A FFT é feita exatamente com os pontos coletados, sem zero padding artificial
        # Isso força os "bins" a terem a largura da resolução
        ...
        # Nota: mode='lines+markers' ajuda a ver onde estão os pontos calculados da FFT
...
with tab3:
    ...
    with col_harm_plot:
        t3 = np.linspace(0, 1.0, 4000) # Mais pontos para melhor resolução visual
        y_h = amp_fund * np.sin(2 * np.pi * f_fund * t3)
        
        # Lógica Cumulativa para Inteiros
        if tipo_harm == "Harmônicos Inteiros (Série)":
            for i in range(2, int(ordem_h) + 1):
                y_h += amp_h * np.sin(2 * np.pi * (f_fund * i) * t3)
        elif tipo_harm != "Nenhum":
            # Para Sub e Inter, adiciona apenas o específico
    ...
    with col_mod_input:
        ...
        n_bandas = st.slider("Quantidade de Pares de Bandas", 1, 10, 1, help="Gera harmônicos da banda lateral: Fc +/- 1Fm, Fc +/- 2Fm...")
        
    with col_mod_plot:
        # Gerar Múltiplas Bandas Laterais (Simulação Matemática Direta)
        # Carrier
        y_mod = 1.0 * np.sin(2 * np.pi * f_portadora * t3)
        
        # Adiciona pares de bandas laterais manualmente para controle total didático
        # Banda Lateral = (Indice / 2) * cos(2pi(Fc - nFm)t) ...
        # Usando seno/cosseno básico para visualização espectral
        for n in range(1, n_bandas + 1):
            amp_banda = indice_mod / (n + 1) # Decaimento natural de amplitude para harmônicos superiores
            # Banda Inferior
        ...
        fig_m.update_layout(title=f"Espectro com Bandas Laterais (Zoom em {f_portadora}Hz)", height=300, 
                            xaxis_range=zoom_range, xaxis_title="Hz")
...
with tab4:
    ...
    with row1_c1:
        ...
        bpfo_order = st.number_input("Ordem de Falha (BPFO)", 1.0, 10.0, 4.2, help="Quantas vezes a esfera bate por giro")
    ...
    with row1_c2:
        ...
        st.caption("Amortecimento: É a taxa de decaimento da energia após o impacto.")
        
        filtro_fc = st.number_input("Filtro Passa-Alta (Hz)", 0, 2000, 500)
    
    # Geração do Sinal Complexo
    fs_env = 8192
    duration_env = 1.0
    ...
    trem[::periodo_idx] = 1.0
    
    # 2. Resposta Impulsiva
    ...
    # 4. Adicionar "Sujeira" Realista (Ruído colorido na banda da ressonância)
    # Isso cria o efeito de "Igrejinha Ruidosa"
    ...
    sinal_bruto += ruido_colorido # Adiciona o morro ruidoso
    sinal_bruto += 0.5 * np.sin(2 * np.pi * (rpm_m4/60) * t_env) # Desbalanceamento
    sinal_bruto += np.random.normal(0, 0.1, len(t_env)) # Ruído de fundo geral
    ...
    fig_raw.add_vrect(x0=fn-200, x1=fn+200, annotation_text="Igrejinha (Ressonância)", fillcolor="orange", opacity=0.2)
